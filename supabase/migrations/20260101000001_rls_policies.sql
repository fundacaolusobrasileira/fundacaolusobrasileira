...
-- Inserção: utilizador só pode criar o seu próprio perfil (matches production)
...
-- Inserção pública (qualquer pessoa pode submeter — matches production)
-- ============================================================

DROP POLICY IF EXISTS "community_media: inserção pública"         ON public.community_media_submissions;
DROP POLICY IF EXISTS "community_media: inserção autenticados"    ON public.community_media_submissions;
DROP POLICY IF EXISTS "community_media: leitura autenticados"     ON public.community_media_submissions;
DROP POLICY IF EXISTS "community_media: leitura"                  ON public.community_media_submissions;
DROP POLICY IF EXISTS "community_media: leitura editor"           ON public.community_media_submissions;
DROP POLICY IF EXISTS "Allow public media submissions"            ON public.community_media_submissions;
DROP POLICY IF EXISTS "community_media: atualização autenticados" ON public.community_media_submissions;
DROP POLICY IF EXISTS "community_media: atualização editor"       ON public.community_media_submissions;
DROP POLICY IF EXISTS "community_media: exclusão autenticados"    ON public.community_media_submissions;
DROP POLICY IF EXISTS "community_media: exclusão editor"          ON public.community_media_submissions;

-- Inserção pública: qualquer pessoa pode submeter mídia (matches production)
CREATE POLICY "community_media: inserção pública"
  ON public.community_media_submissions FOR INSERT
  WITH CHECK (true);

-- Leitura: só editores/admin (matches production)
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'partners_name_length') THEN
    ALTER TABLE public.partners ADD CONSTRAINT partners_name_length CHECK (length(trim(name)) >= 2);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'partners_type_check') THEN
    ALTER TABLE public.partners ADD CONSTRAINT partners_type_check CHECK (type IN ('pessoa', 'empresa'));
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'partners_category_check') THEN
    ALTER TABLE public.partners ADD CONSTRAINT partners_category_check CHECK (category IN ( 'Parceiro Platinum', 'Parceiro Gold', 'Parceiro Silver', 'Apoio Público', 'Outro Apoio', 'Exposição', 'Governança' ));
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'events_title_length') THEN
    ALTER TABLE public.events ADD CONSTRAINT events_title_length CHECK (length(trim(title)) >= 2);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'events_status_check') THEN
    ALTER TABLE public.events ADD CONSTRAINT events_status_check CHECK (status IN ('draft', 'published'));
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'events_category_check') THEN
    ALTER TABLE public.events ADD CONSTRAINT events_category_check CHECK (category IN ('33 Anos', 'Fundação', 'Embaixada', 'Outros'));
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'precadastros_email_format') THEN
    ALTER TABLE public.precadastros ADD CONSTRAINT precadastros_email_format CHECK (email ~* '^[^@\s]+@[^@\s]+\.[^@\s]+$');
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'precadastros_name_length') THEN
    ALTER TABLE public.precadastros ADD CONSTRAINT precadastros_name_length CHECK (length(trim(name)) >= 2);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'precadastros_status_check') THEN
    ALTER TABLE public.precadastros ADD CONSTRAINT precadastros_status_check CHECK (status IN ('novo', 'contatado', 'aprovado', 'rejeitado', 'convertido'));
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'precadastros_registration_type_check') THEN
    ALTER TABLE public.precadastros ADD CONSTRAINT precadastros_registration_type_check CHECK ( "registrationType" IS NULL OR "registrationType" IN ('membro', 'parceiro', 'colaborador', 'embaixador') );
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'community_media_email_format') THEN
    ALTER TABLE public.community_media_submissions ADD CONSTRAINT community_media_email_format CHECK (email ~* '^[^@\s]+@[^@\s]+\.[^@\s]+$');
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'community_media_author_name_length') THEN
    ALTER TABLE public.community_media_submissions ADD CONSTRAINT community_media_author_name_length CHECK (length(trim(author_name)) >= 2);
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'community_media_type_check') THEN
    ALTER TABLE public.community_media_submissions ADD CONSTRAINT community_media_type_check CHECK (type IN ('image', 'video'));
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'community_media_status_check') THEN
    ALTER TABLE public.community_media_submissions ADD CONSTRAINT community_media_status_check CHECK (status IN ('pending', 'approved', 'rejected'));
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'community_media_url_not_empty') THEN
    ALTER TABLE public.community_media_submissions ADD CONSTRAINT community_media_url_not_empty CHECK (length(trim(url)) > 0);
  END IF;
END $$;
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT FROM pg_constraint WHERE conname = 'profiles_role_check') THEN
    ALTER TABLE public.profiles ADD CONSTRAINT profiles_role_check CHECK (role IN ('membro', 'editor', 'admin'));
  END IF;
END $$;
