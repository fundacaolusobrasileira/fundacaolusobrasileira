...
export const HomePage = () => {
  ...
  const boardSource = PARTNERS.some(p => p.tier) ? PARTNERS : MEMBERS_SEED as any[];
  const presidente = boardSource.filter((m: any) => m.tier === 'presidente');
  const direcao = boardSource.filter((m: any) => m.tier === 'direcao');
  const secretarioGeral = boardSource.filter((m: any) => m.tier === 'secretario-geral');
  const vogais = boardSource.filter((m: any) => m.tier === 'vogal');
